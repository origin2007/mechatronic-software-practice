...
//Tasks
Task t4();
Task t1(7000, 10, &t1Callback);
Task t2(6000, TASK_FOREVER, &t2Callback);
Task t3(8000, TASK_FOREVER, &t3Callback);

Scheduler runner;

void t1Callback() {
    Serial.print("t1: ");
    Serial.println(millis());//输出当前时间
    ...
    if (t1.isLastIteration()) {
      t3.disable();
      runner.deleteTask(t3);//删除任务
      t2.setInterval(500);//设置时间
